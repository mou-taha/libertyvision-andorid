...
public class HalloweenActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog = new ProgressDialog(HalloweenActivity.this, true);
    ...
    private void setupBackPressHandler() {
        OnBackPressedCallback callback = new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                DialogUtil.exitDialog(HalloweenActivity.this);
            }
        };
    ...
    private void setAds() {
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            ...
            DialogUtil.popupAdsDialog(HalloweenActivity.this);
        }, 600);

        if (DeviceUtils.isTvBox(this)){
            return;
        }
        new GDPRChecker(HalloweenActivity.this).check();
    ...
    private void setListenerHome() {
        ...
        findViewById(R.id.iv_notifications).setOnClickListener(v -> startActivity(new Intent(HalloweenActivity.this, NotificationsActivity.class)));
        findViewById(R.id.iv_file_download).setOnClickListener(v -> startActivity(new Intent(HalloweenActivity.this, DownloadActivity.class)));
        findViewById(R.id.iv_profile).setOnClickListener(v -> startActivity(new Intent(HalloweenActivity.this, ProfileActivity.class)));
        findViewById(R.id.iv_profile_re).setOnClickListener(v -> signOut());
        findViewById(R.id.iv_settings).setOnClickListener(v -> startActivity(new Intent(HalloweenActivity.this, SettingActivity.class)));
        ...
        findViewById(R.id.select_epg).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(HalloweenActivity.this, CategoriesActivity.class));
            }
        });
        findViewById(R.id.select_multiple_screen).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(HalloweenActivity.this, MultipleScreenActivity.class));
            }
        });
        findViewById(R.id.select_catch_up).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(HalloweenActivity.this, CatchUpActivity.class));
            }
        });
        findViewById(R.id.iv_radio).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(HalloweenActivity.this, RadioActivity.class));
            }
        });
    ...
    private void setAutoRenew() {
        findViewById(R.id.ll_tv_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
                DialogUtil.downloadDataDialog(HalloweenActivity.this, Callback.TAG_TV, type -> getLive());
            } else {
                getLive();
            }
        });
        findViewById(R.id.ll_movie_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.downloadDataDialog(HalloweenActivity.this, Callback.TAG_MOVIE, type -> getMovies());
            } else {
                getMovies();
            }
        });
        findViewById(R.id.ll_series_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.downloadDataDialog(HalloweenActivity.this, Callback.TAG_SERIES, type -> getSeries());
            } else {
                getSeries();
            }
        });
    ...
    private void setLongClickListener() {
        ...
        findViewById(R.id.select_movie).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.downloadDataDialog(HalloweenActivity.this, Callback.TAG_TV, type -> getMovies());
            }
            return false;
        });
        findViewById(R.id.select_serials).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.downloadDataDialog(HalloweenActivity.this, Callback.TAG_TV, type -> getSeries());
            }
            return false;
        });
    }
    ...
    private void selectSerials() {
        if (spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
            ...
            startActivity(new Intent(HalloweenActivity.this, SeriesActivity.class));
        }
    }
    ...
    private void selectMovie() {
        if (spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
            ...
            Intent intent = new Intent(HalloweenActivity.this, MovieActivity.class);
    ...
    private void selectLive() {
        if (spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
            ...
            Intent intent = new Intent(HalloweenActivity.this, LiveTvActivity.class);
    ...
    private void signOut() {
        DialogUtil.logoutDialog(HalloweenActivity.this, () -> {
            Intent intent = new Intent(HalloweenActivity.this, UsersListActivity.class);
            if (spHelper.isLogged()) {
                new JSHelper(HalloweenActivity.this).removeAllData();
                spHelper.removeSignOut();
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                intent.putExtra("from", "");
                Toast.makeText(HalloweenActivity.this, getString(R.string.logout_success), Toast.LENGTH_SHORT).show();
            } else {
    ...
    private void getSeries() {
        if (!NetworkUtils.isConnected(this)){
            pbSerials.setVisibility(View.GONE);
            Toasty.makeText(HalloweenActivity.this,true, getString(R.string.err_internet_not_connected), Toasty.ERROR);
            return;
        }
    ...
    private void handlerSeriesEnd(String success, String msg) {
        if (success.equals("1")) {
            ...
            spHelper.setCurrentDate(Callback.TAG_SERIES);
            changeIconView(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,false);
            Toast.makeText(HalloweenActivity.this, getString(R.string.added_success), Toast.LENGTH_SHORT).show();
        }  else {
            spHelper.setCurrentDateEmpty(Callback.TAG_SERIES);
            changeIconView(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,true);
            pbSerials.setVisibility(View.GONE);
            if (success.equals("3")){
                Toasty.makeText(HalloweenActivity.this,true, msg, Toasty.ERROR);
            } else {
                Toast.makeText(HalloweenActivity.this, getString(R.string.err_server_not_connected), Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public int setContentViewID() {
        return R.layout.activity_themes_halloween;
    }
