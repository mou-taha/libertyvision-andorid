...
public class SportsActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog = new ProgressDialog(SportsActivity.this,true);
    ...
    private void setupBackPressHandler() {
        OnBackPressedCallback callback = new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                DialogUtil.exitDialog(SportsActivity.this);
            }
        };
    ...
    private void setAds() {
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            ...
            DialogUtil.popupAdsDialog(SportsActivity.this);
        }, 600);

        if (DeviceUtils.isTvBox(this)){
            return;
        }
        new GDPRChecker(SportsActivity.this).check();
    ...
    private void getPoster() {
        ...
        new LoadPoster(new PosterListener() {
            ...
            public void onEnd(String success, String verifyStatus, String message, ArrayList<ItemPoster> arrayList) {
                ...
                if (success.equals("1") && (!arrayList.isEmpty())){
                    ...
                    int themeBg = ThemeHelper.getThemeBackgroundRes(SportsActivity.this);
                    Picasso.get()
        ...
        }, new Helper(SportsActivity.this).getAPIRequestNSofts(Method.METHOD_POSTER,
                "sports_ui", "", "", "")
        ).execute();
    }
    ...
    private void setListenerHome() {
        ...
        findViewById(R.id.iv_notifications).setOnClickListener(v -> startActivity(new Intent(SportsActivity.this, NotificationsActivity.class)));
        findViewById(R.id.iv_file_download).setOnClickListener(v -> startActivity(new Intent(SportsActivity.this, DownloadActivity.class)));
        findViewById(R.id.iv_profile).setOnClickListener(v -> startActivity(new Intent(SportsActivity.this, ProfileActivity.class)));
        findViewById(R.id.iv_profile_re).setOnClickListener(v -> signOut());
        findViewById(R.id.iv_settings).setOnClickListener(v -> startActivity(new Intent(SportsActivity.this, SettingActivity.class)));
        ...
        findViewById(R.id.select_epg).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(SportsActivity.this, CategoriesActivity.class));
            }
        });
        findViewById(R.id.select_multiple_screen).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(SportsActivity.this, MultipleScreenActivity.class));
            }
        });
        findViewById(R.id.select_catch_up).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(SportsActivity.this, CatchUpActivity.class));
            }
        });
        findViewById(R.id.iv_radio).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(SportsActivity.this, RadioActivity.class));
            }
        });
    ...
    private void setAutoRenew() {
        findViewById(R.id.ll_tv_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
                DialogUtil.downloadDataDialog(SportsActivity.this, Callback.TAG_TV, type -> getLive());
            } else {
                getLive();
            }
        });
        findViewById(R.id.ll_movie_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.downloadDataDialog(SportsActivity.this, Callback.TAG_MOVIE, type -> getMovies());
            } else {
                getMovies();
            }
        });
        findViewById(R.id.ll_series_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.downloadDataDialog(SportsActivity.this, Callback.TAG_SERIES, type -> getSeries());
            } else {
                getSeries();
            }
        });
    ...
    private void setLongClickListener() {
        ...
        findViewById(R.id.select_movie).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.downloadDataDialog(SportsActivity.this, Callback.TAG_TV, type -> getMovies());
            }
            return false;
        });
        findViewById(R.id.select_serials).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.downloadDataDialog(SportsActivity.this, Callback.TAG_TV, type -> getSeries());
            }
            return false;
        });
    }
    ...
    private void selectSerials() {
        if (spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
            ...
            startActivity(new Intent(SportsActivity.this, SeriesActivity.class));
        }
    }
    ...
    private void selectMovie() {
        if (spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
            ...
            Intent intent = new Intent(SportsActivity.this, MovieActivity.class);
    ...
    private void selectLive() {
        if (spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
            ...
            Intent intent = new Intent(SportsActivity.this, LiveTvActivity.class);
    ...
    private void signOut() {
        DialogUtil.logoutDialog(SportsActivity.this, () -> {
            Intent intent = new Intent(SportsActivity.this, UsersListActivity.class);
            if (spHelper.isLogged()) {
                new JSHelper(SportsActivity.this).removeAllData();
                spHelper.removeSignOut();
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                intent.putExtra("from", "");
                Toast.makeText(SportsActivity.this, getString(R.string.logout_success), Toast.LENGTH_SHORT).show();
            } else {
    ...
    private void getSeries() {
        if (!NetworkUtils.isConnected(this)){
            pbSerials.setVisibility(View.GONE);
            Toasty.makeText(SportsActivity.this,true, getString(R.string.err_internet_not_connected), Toasty.ERROR);
            return;
        }
    ...
    private void handlerSeriesEnd(String success, String msg) {
        if (success.equals("1")) {
            ...
            spHelper.setCurrentDate(Callback.TAG_SERIES);
            changeIconView(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,false);
            Toast.makeText(SportsActivity.this, getString(R.string.added_success), Toast.LENGTH_SHORT).show();
        }  else {
            spHelper.setCurrentDateEmpty(Callback.TAG_SERIES);
            changeIconView(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,true);
            pbSerials.setVisibility(View.GONE);
            if (success.equals("3")){
                Toasty.makeText(SportsActivity.this,true, msg, Toasty.ERROR);
            } else {
                Toast.makeText(SportsActivity.this, getString(R.string.err_server_not_connected), Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public int setContentViewID() {
        return R.layout.activity_themes_sports;
    }
